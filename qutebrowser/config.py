...
c.colors.completion.fg = [base08, base08, base08]
...
# Type: QtColor
c.colors.downloads.error.bg = base01
...
# Type: ShellCommand
c.fileselect.single_file.command = ['termite', '-e', 'ranger --choosefile={}']
...
# Type: String
c.hints.border = '1px solid ' + base08
...
# Type: List of Regex
c.hints.next_regexes = ['\\bsui(vant)?\\b', '\\bnext\\b', '\\bmore\\b', '\\bnewer\\b', '\\b[>→≫]\\b', '\\b(>>|»)\\b', '\\bcontinue\\b']
...
# Type: List of Regex
c.hints.prev_regexes = ['\\bprec(edent)?\\b', '\\bprev(ious)?\\b', '\\bback\\b', '\\bolder\\b', '\\b[<←≪]\\b', '\\b(<<|«)\\b']
...
c.url.searchengines = {
    ...
    'wfr': 'https://fr.wikipedia.org/wiki/Special:Search?search={}',
    'trans': 'https://translate.google.com/?source=osdd#auto|auto|{}',
    'enfr': 'https://www.wordreference.com/redirect/translation.aspx?dict=enfr&w={}',
    'fren': 'https://www.wordreference.com/redirect/translation.aspx?dict=fren&w={}',
    'deen': 'https://fr.pons.com/traduction?l=deen&in=de&lf=de&qnac=&q={}',
    'ende': 'https://fr.pons.com/traduction?l=deen&in=en&lf=en&qnac=&q={}',
    'defr': 'https://fr.pons.com/traduction?l=defr&in=fr&lf=fr&qnac=&q={}',
    'frde': 'https://fr.pons.com/traduction?l=defr&in=de&lf=de&qnac=&q={}'
}
...
config.bind('hm', 'hint links spawn mpv {hint-url}')
...
config.bind('hf', 'hint all tab-fg')
...
config.bind('aq', 'download-cancel')
# config.bind('b', 'set-cmd-text -s :quickmark-load')
config.bind('ad', 'download-clear')
config.bind('ao', 'tab-only')
...
config.bind('t', 'run-with-count 6 scroll down')
config.bind('s', 'run-with-count 6 scroll up')
...
config.bind('xf', 'save')
config.bind('xk', 'set-cmd-text -s :bind')
config.bind('xl', 'set-cmd-text -s :set -t')
config.bind('xs', 'set-cmd-text -s :set')
config.bind('jCH', 'config-cycle -p -u *://*.{url:host}/* content.cookies.accept all no-3rdparty never ;; reload')
config.bind('jCh', 'config-cycle -p -u *://{url:host}/* content.cookies.accept all no-3rdparty never ;; reload')
config.bind('jCu', 'config-cycle -p -u {url} content.cookies.accept all no-3rdparty never ;; reload')
config.bind('jIH', 'config-cycle -p -u *://*.{url:host}/* content.images ;; reload')
config.bind('jIh', 'config-cycle -p -u *://{url:host}/* content.images ;; reload')
config.bind('jIu', 'config-cycle -p -u {url} content.images ;; reload')
...
config.bind('jcH', 'config-cycle -p -t -u *://*.{url:host}/* content.cookies.accept all no-3rdparty never ;; reload')
config.bind('jch', 'config-cycle -p -t -u *://{url:host}/* content.cookies.accept all no-3rdparty never ;; reload')
config.bind('jcu', 'config-cycle -p -t -u {url} content.cookies.accept all no-3rdparty never ;; reload')
config.bind('jh', 'back -t')
config.bind('jiH', 'config-cycle -p -t -u *://*.{url:host}/* content.images ;; reload')
config.bind('jih', 'config-cycle -p -t -u *://{url:host}/* content.images ;; reload')
config.bind('jiu', 'config-cycle -p -t -u {url} content.images ;; reload')
...
config.bind('q', 'mode-enter normal', mode='caret')
